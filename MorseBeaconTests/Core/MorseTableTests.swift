...
final class MorseTableTests: XCTestCase {
    ...
    func test_everySupportedNonSpaceCharacterHasSymbols() {
        ...
        let candidates: [Character] = {
            var out: [Character] = [" "]
            for s in UnicodeScalar("A").value...UnicodeScalar("Z").value { out.append(Character(UnicodeScalar(s)!)) }
            for s in UnicodeScalar("0").value...UnicodeScalar("9").value { out.append(Character(UnicodeScalar(s)!)) }
            out += [".", ",", "?", "'", "!", "/", "(", ")", "&", ":", ";", "=", "+", "-", "_", "\"", "$", "@"]
            return out
        }()
